...
from .util import (dict_to_bytes, bytes_to_dict, bytes_to_hexstr, estimate_free_space)
...
class Receiver:
    ...
    def _go(self, w):
        ...
        def on_slow_verification():
            print("Key established, waiting for confirmation...",
    ...
    def _handle_directory(self, them_d):
        ...
        if zipmode != "zipfile/deflated":
            self._msg("Error: unknown directory-transfer mode '%s'" % (zipmode,))
        ...
        if free is not None and free < file_data["numbytes"]:
            self._msg("Error: insufficient free space (%sB) for directory (%sB)"
    ...
    def _decide_destname(self, mode, destname):
        ...
        if os.path.exists(abs_destname):
            if self.args.output_file: # overwrite is intentional
                self._msg("Overwriting '%s'" % destname)
                if self.args.accept_file:
                    self._remove_existing(abs_destname)
            else:
                self._msg("Error: refusing to overwrite existing '%s'" % destname)
                raise TransferRejectedError()
        return abs_destname
    ...
    def _write_file(self, f):
        ...
        self._msg("Received file written to %s" %
    ...
    def _write_directory(self, f):
        ...
        with self.args.timing.add("unpack zip"):
            ...
            self._msg("Received files written to %s/" %
                      os.path.basename(self.abs_destname))
            f.close()
